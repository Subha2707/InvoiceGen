...
export const calculateLineItemGST = (item, sellerStateCode, clientStateCode) => {
  ...
  const gstAmount = parseFloat(((taxableAmount * Number(item.gstPercentage)) / 100).toFixed(2));
  ...
  if (isInterState) {
    igstRate = Number(item.gstPercentage);
    igstAmount = gstAmount;
  } else {
    cgstRate = Number(item.gstPercentage) / 2;
    sgstRate = Number(item.gstPercentage) / 2;
...
export const calculateInvoiceTotals = (items, discount, shippingCharge, sellerStateCode, clientStateCode) => {
  const isInterState = sellerStateCode !== clientStateCode;
  let subtotal = 0;
  let totalCgst = 0;
  let totalSgst = 0;
  ...
  if (discount && discount.value) {
    if (discount.type === 'percentage') {
      ...
      discountAmount = parseFloat(Number(discount.value).toFixed(2));
    }
  }
